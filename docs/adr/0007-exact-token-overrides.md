...
Product ink, borders, Black, and Outline Shadow are exact theme values on Reshaped semantic slots — not derived from `generateThemeColors({ neutral })` (that seed paints the neutral **surface** family) and not left on stock when the mock diverges. Stock `borderNeutral` / `borderNeutralFaded` / `black` / `shadow.outline` also miss the product (translucent/chrome borders; true black; multi-stop outline shadow).
...
- Exact-override Neutral Border and Neutral Border Faded (`borderNeutral`, `borderNeutralFaded`) when the mock diverges from stock; leave other non-outline shadow slots on stock until a distinct product value shows up.
...
- Decorative separators (`Divider` default) consume Neutral Border Faded — not Neutral Border.
...
- **Stretch Neutral Border to cover separators** — rejected; interactive chrome and decorative hairlines diverge in the mock.
- **Override raised / overlay shadows by default** — other elevations may differ; leave stock until needed.
