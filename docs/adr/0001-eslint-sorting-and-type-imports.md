...
We use `eslint-plugin-perfectionist`’s full **`recommended-natural`** preset (latest), plus three overrides we care about. Sorting is meant to be **autofixable** and boring on save — if the editor fixes it, churn is fine.
...
- Don’t “simplify away” unsorted functions or named-then-nullish without revisiting this ADR — those look random in config without this note.
