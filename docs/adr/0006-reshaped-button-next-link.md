...
`render` is a function prop, so `LinkButton` is `"use client"` and callers must be in the client graph to pass anything non-serializable. An SVG component is a function: a wrapper passing `icon` marks itself `"use client"` and imports its own asset (`ui/create-new-button`), and `ui/app-header` is a client component for the same reason. A server-rendered caller may still use `LinkButton` with serializable props only.
