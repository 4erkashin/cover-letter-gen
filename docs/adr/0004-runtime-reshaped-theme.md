...
**Decision:** define the `altShift` theme in `ui/theme.ts`, generate CSS at runtime with `baseThemeDefinition` + `generateThemeColors({ primary: PRIMARY, brand: BRAND })`, inject that CSS from the server root layout, and point `Reshaped` / `data-rs-theme` at `altShift`. Do not add a `reshaped theming` build or watch script unless we later need CLI-only outputs (e.g. custom viewport media).
...
- Hex seeds live only as `BRAND` / `PRIMARY` in the theme entry — not duplicated in `CONTEXT.md`.
